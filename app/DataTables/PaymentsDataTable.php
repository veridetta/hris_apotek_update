...
class PaymentsDataTable extends DataTable
{
    ...
    {
        $now = Carbon::now();
        $data = Schedule::select('attendances.lembur as lembur','attendances.id','attendances.at_in as attendances.at_in','attendances.at_out as attendances.at_out','shifts.in as shifts.in','shifts.out as shifts.out','attendances.status as attendances.status','schedules.dates')->join('attendances','attendances.schedules_id','=','schedules.id')->join('shifts','shifts.id','=','schedules.shifts_id')->join('employees','employees.id','=','attendances.employees_id')->where('employees.id',$this->attributes['id'])->whereBetween('schedules.dates', [$this->attributes['from'], $this->attributes['to']]);
